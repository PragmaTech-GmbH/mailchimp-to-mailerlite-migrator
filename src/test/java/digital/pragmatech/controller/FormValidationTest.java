...
class FormValidationTest {
    ...
    void shouldDisplayPlaceholderTexts() throws Exception {
        ...
        assertThat(mailerLiteInput.getAttribute("placeholder"))
                .isEqualTo("Your MailerLite API token");
    ...
    void shouldDisplayHelpTexts() throws Exception {
        ...
        // Then
        HtmlElement mailchimpHelp = page.getFirstByXPath("//p[contains(text(), 'Find your API key in Mailchimp')]");
        HtmlElement mailerLiteHelp = page.getFirstByXPath("//p[contains(text(), 'Find your API token in MailerLite')]");
        
        assertThat(mailchimpHelp.getTextContent()).contains("Account → Extras → API keys");
        assertThat(mailerLiteHelp.getTextContent()).contains("Integrations → MailerLite API → Generate new token");
    ...
    void shouldShowValidationSectionWhenApiKeysConfigured() throws Exception {
        ...
        // Then
        HtmlElement validationSection = page.getFirstByXPath("//div[contains(@class, 'bg-white') and .//h3[contains(text(), 'Validate API Connections')]]");

...
class ApiEndpointsIntegrationTest {
    ...
    void shouldHandleCompleteApiWorkflow() throws Exception {
        ...
        mockMvc.perform(post("/api/configure")
        ...
        MigrationValidator.ValidationResult validationResult = MigrationValidator.ValidationResult.builder()
                .valid(true)
                .errors(List.of())
        ...
        MigrationValidator.PreMigrationAnalysis analysis = MigrationValidator.PreMigrationAnalysis.builder()
        ...
        when(migrationValidator.analyzeForMigration())
        ...
        when(migrationOrchestrator.startMigration())
                .thenReturn(CompletableFuture.completedFuture("migration-123"));
    ...
    void shouldHandleAnalysisWithErrors() throws Exception {
        ...
        mockMvc.perform(post("/api/analyze"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.message").value("Analysis failed: Failed to connect to Mailchimp API"));
    }

    @Test
    void shouldHandleTagsFetchFailure() throws Exception {
        // Given
        when(mailchimpService.getAllLists())
    ...
    void shouldHandleMigrationStateTransitions() throws Exception {
        ...
        mockMvc.perform(post("/api/migration/pause"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
        ...
        mockMvc.perform(post("/api/migration/resume"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
        ...
        mockMvc.perform(post("/api/migration/cancel"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
    }
    ...
    void shouldHandleApiValidationWithTimeout() throws Exception {
        ...
        mockMvc.perform(post("/api/validate"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.message").value("Validation failed: java.lang.RuntimeException: Connection timeout after 30 seconds"));
    }
    ...
    private MigrationStatus createDetailedMigrationStatus() {
        MigrationStatus.Progress progress = MigrationStatus.Progress.builder()
                .totalItems(2000)
                .processedItems(750)
                .successfulItems(745)
                .failedItems(5)
        ...
        MigrationStatus.Statistics statistics = MigrationStatus.Statistics.builder()
